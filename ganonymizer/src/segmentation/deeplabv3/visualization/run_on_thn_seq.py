...
network = DeepLabV3("eval_seq_thn", project_dir="my_deeplabv3").to(device)
network.load_state_dict(torch.load("my_deeplabv3/pretrained_models/model_13_2_2_2_epoch_580.pth", map_location=device))

val_dataset = DatasetThnSeq(thn_data_path="my_deeplabv3/data/thn")
...
for step, (imgs, img_ids) in enumerate(val_loader):
    with torch.no_grad(): # (corresponds to setting volatile=True in all variables, this is done during inference to reduce memory consumption)
        ...
        print(pred_label_imgs)
        print(pred_label_imgs.shape)
