...
class DeepLabV3(nn.Module):
    ...
    def __init__(self, res_type, res_path, device):
        ...
        if res_type == 18:
            ...
            self.resnet = ResNet18_OS8(os.path.join(res_path, weight_name), device) # NOTE! specify the type of ResNet here
            ...
            self.resnet = ResNet18_OS8(os.path.join(res_path, weight_name), device)
            self.aspp = ASPP(num_classes=self.num_classes)
        elif res_type == 50:
            weight_name = "resnet50-19c8e357.pth"
            self.resnet = ResNet18_OS50(os.path.join(res_path, weight_name), device)
            self.aspp = ASPP_Bottleneck(num_classes=self.num_classes)
        elif res_type == 101:
            weight_name = "resnet101-5d3b4d8f.pth"
            self.resnet = ResNet18_OS101(os.path.join(res_path, weight_name), device)
            self.aspp = ASPP_Bottleneck(num_classes=self.num_classes)
        elif res_type == 152:
            weight_name = "resnet152-b121ed2d.pth"
            self.resnet = ResNet18_OS152(os.path.join(res_path, weight_name), device)
            self.aspp = ASPP_Bottleneck(num_classes=self.num_classes)
